...
tail decode, not by the utterance length. Since the same graphs run, streaming
output is unchanged; the only approximation is that cached windows keep the
peak normalization and log-mel floor that were in effect when they completed.
