...
| **Streaming** | ✅ Works | SSE PCM chunks at native 16 kHz; requires `retry_badcase=false`. On CPU, CUDA, HIP and Vulkan each patch is decoded with the AudioVAE's causal-convolution state carried over from the previous patch, so patch boundaries match the offline decode. Metal still decodes each patch on its own and clicks at patch boundaries. |
