...
On a terminal the transcript is rewritten in place on one line as it grows; when stdout is
redirected, each update is written as a separate `partial_text=` line so pipes stay parseable.
