...
engine::core::BackendType parse_backend(const std::string & value) {
    ...
    throw std::runtime_error("IndexTTS2.5 warmbench backend must be cuda or cpu");
}
...
std::vector<std::pair<std::string, std::string>> parse_session_options(int argc, char ** argv) {
    ...
    for (int i = 1; i + 1 < argc; ++i) {
        ...
        if (eq == std::string::npos || eq == 0) {
            throw std::runtime_error("invalid IndexTTS2.5 --session-option: " + option);
        }
    ...
    if (value.is_array()) {
        ...
        for (size_t i = 0; i < items.size(); ++i) {
            ...
            if (!items[i].is_number()) {
                throw std::runtime_error("IndexTTS2.5 warmbench option arrays must contain only numbers");
            }
...
std::string required_string(const engine::io::json::Value & object, const std::string & key) {
    ...
    if (value == nullptr || value->is_null()) {
        throw std::runtime_error("IndexTTS2.5 warmbench request missing " + key);
    }
    return value->as_string();
}
...
std::vector<engine::runtime::TaskRequest> parse_requests(const std::string & request_sequence_json) {
    if (request_sequence_json.empty()) {
        throw std::runtime_error("IndexTTS2.5 warmbench requires --request-sequence-json");
    }
    ...
    if (requests.empty()) {
        throw std::runtime_error("IndexTTS2.5 warmbench request sequence is empty");
    }
    return requests;
}
...
engine::io::json::Value audio_summary_json(const engine::runtime::AudioBuffer & audio) {
    if (audio.samples.empty()) {
        throw std::runtime_error("IndexTTS2.5 warmbench received empty audio output");
    }
    double sum = 0.0;
    double abs_sum = 0.0;
    double sq_sum = 0.0;
...
int main(int argc, char ** argv) {
    try {
        const std::filesystem::path model_path = arg_value(argc, argv, "--model", "models/IndexTTS-2.5");
        ...
        const std::filesystem::path timing_path = arg_value(argc, argv, "--timing-file", "/tmp/index_tts2_5_warm_bench_timing.log");
        ...
        load_request.family_hint = "index_tts2_5";
        ...
        if (session == nullptr) {
            throw std::runtime_error("IndexTTS2.5 model did not create an offline voice task session");
        }
        ...
        timing_lines.push_back("index_tts2_5.backend " + backend_name);
        timing_lines.push_back("index_tts2_5.model_root " + model_path.string());
        ...
        for (size_t request_index = 0; request_index < requests.size(); ++request_index) {
            ...
            for (int iteration = 0; iteration < std::max(1, iterations); ++iteration) {
                ...
                total_ms += wall_ms;
                timing_lines.push_back("index_tts2_5.wall_ms " + engine::io::json::stringify_number(wall_ms));
            }
            if (!last_result.audio_output.has_value()) {
                throw std::runtime_error("IndexTTS2.5 warmbench expected audio output");
            }
            ...
            std::cout << "index_tts2_5.wall_ms=" << avg_ms << "\n";
        }
        ...
        std::cerr << "index_tts2_5_warm_bench failed: " << ex.what() << "\n";
        return 1;
    }
}
