...
def advisory(message: str) -> str:
    return f"{message} (advisory; --strict-catalog makes this fail)"
...
def main() -> int:
    ...
    parser.add_argument(
        "--strict-catalog",
        action="store_true",
        help="Fail on WebUI catalog and model_params findings instead of reporting them as advisories",
    )
    ...
    # The catalog pass is new, and the catalog it inspects predates it. Reporting
    # its findings as advisories by default means this check can land without
    # rewriting the data it measures; --strict-catalog is what CI turns on once
    # the catalog corrections have followed.
    if args.strict_catalog:
        errors.extend(catalog_sync_errors)
    else:
        warnings.extend(advisory(error) for error in catalog_sync_errors)
    ...
    if args.strict_catalog:
        ...
        warnings.extend(advisory(error) for error in params_errors)
    ...
    advisories = sum(1 for warning in warnings if warning.endswith("--strict-catalog makes this fail)"))
    if advisories:
        print(
            f"ok: runtime loaders, model_specs and model_manager_v2 are in sync; "
            f"{advisories} WebUI catalog advisories reported (--strict-catalog fails on them)"
        )
    else:
