...
engine::runtime::TaskResult run_streaming_from_stdin(
    ...
    minitts::app::set_stdin_binary_mode();
    std::cout << "audio_input=stdin format=" << minitts::app::to_string(sample_format)
              << " rate=" << format.sample_rate
              << " channels=" << format.channels << "\n"
              << std::flush;
    const auto stream = minitts::app::make_pcm_chunk_stream(std::cin, format, sample_format);
...
void run_streaming(
    ...
    // Partials carry the full transcript so far. On a terminal, rewrite one line in place so the
    // reader sees the current transcript rather than a pyramid of growing copies. When stdout is
    // redirected, keep one line per update so pipes and logs stay parseable.
    const bool interactive_output = isatty(fileno(stdout)) != 0;
    size_t rendered_width = 0;
    const minitts::app::StreamEventSink sink =
        [&](const engine::runtime::StreamEvent & event) {
            if (event.partial_text.has_value()) {
                const std::string & text = event.partial_text->text;
                if (interactive_output) {
                    std::cout << '\r' << text;
                    // Blank out whatever the previous, longer line left behind.
                    if (text.size() < rendered_width) {
                        std::cout << std::string(rendered_width - text.size(), ' ');
                    }
                    rendered_width = text.size();
                    std::cout << std::flush;
                } else {
                    // Flushed so partials appear as they are produced, including when piped.
                    std::cout << "partial_text=" << text << "\n" << std::flush;
                }
            }
    ...
    if (interactive_output && rendered_width > 0) {
        // Close the in-place line so the summary below starts on a fresh row.
        std::cout << "\n";
    }
