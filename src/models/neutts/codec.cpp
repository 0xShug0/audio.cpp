    ...
    switch (storage_type) {
        ...
        default:
            throw std::runtime_error(
                "NeuTTS codec weight_type supports only native, f32, f16, bf16, and q8_0");
    }
}
...
void validate_conv_storage_type(assets::TensorStorageType storage_type) {
    switch (storage_type) {
        ...
        default:
            throw std::runtime_error(
                "NeuTTS codec_conv_weight_type supports only native, f32, and f16");
    }
}
...
NeuTTSCodecResnetBlockWeights load_resnet_block(
    core::BackendWeightStore & store,
    const assets::TensorSource & source,
    const std::string & prefix,
    int64_t channels,
    assets::TensorStorageType conv_storage_type) {
    NeuTTSCodecResnetBlockWeights out;
...
NeuTTSCodecTransformerLayerWeights load_transformer_layer(
    core::BackendWeightStore & store,
    const assets::TensorSource & source,
    const std::string & prefix,
    ...
    NeuTTSCodecTransformerLayerWeights out;
...
int64_t fsq_codebook_size(const std::vector<int64_t> & levels) {
    ...
    for (const int64_t level : levels) {
        if (level <= 1) {
            throw std::runtime_error("NeuTTS FSQ levels must be greater than one");
        }
        size *= level;
    }
    return size;
...
std::shared_ptr<const NeuTTSAssets> require_assets(
    ...
    if (assets == nullptr) {
        throw std::runtime_error("NeuTTS codec runtime requires assets");
    }
    return assets;
}
...
core::TensorValue group_norm(
    ...
    if (x.shape.rank != 3 || groups <= 0 || x.shape.dims[1] % groups != 0) {
        throw std::runtime_error("NeuTTS codec GroupNorm requires [batch, channels, frames]");
    }
    ...
    auto grouped = core::reshape_tensor(
        ...
        grouped_shape);
    auto mean = modules::ReduceMeanModule({2}).build(ctx, grouped);
    ...
    auto weight = core::reshape_tensor(ctx, *weights.weight, core::TensorShape::from_dims({1, channels, 1}));
    auto bias = core::reshape_tensor(ctx, *weights.bias, core::TensorShape::from_dims({1, channels, 1}));
...
core::TensorValue add_tensors(
    ...
    core::validate_shape(rhs, lhs.shape, "residual");
    auto lhs_contiguous = core::ensure_backend_addressable_layout(ctx, lhs);
    auto rhs_contiguous = core::ensure_backend_addressable_layout(ctx, rhs);
...
core::TensorValue transformer_layer(
    ...
    x = apply_codec_attention(
        ...
        config,
        positions);
    auto hidden = add_tensors(ctx, input, x);
    ...
    return add_tensors(ctx, hidden, x);
}
...
public:
    CodecWeightsRuntime(
        ...
        : assets_(require_assets(std::move(assets))),
          ...
          weights_(std::make_shared<NeuTTSCodecDecoderWeights>(
              load_neutts_codec_decoder_weights(
                  *assets_,
                  backend_,
                  backend_type_,
                  ...
                  conv_storage_type))) {
        if (backend_ == nullptr) {
            throw std::runtime_error("NeuTTS codec backend is not initialized");
        }
    }

    const NeuTTSAssets & assets() const noexcept { return *assets_; }
    const NeuTTSCodecDecoderWeights & weights() const noexcept { return *weights_; }
...
private:
    ...
    int device_ = 0;
    int threads_ = 1;
    std::shared_ptr<const NeuTTSCodecDecoderWeights> weights_;
};
...
public:
    CodecHeadGraph(
        ...
        : runtime_(std::move(runtime)),
          frames_(frames) {
        if (frames_ <= 0) {
            throw std::runtime_error("NeuTTS codec requires positive frame count");
        }
        const auto build_start = Clock::now();
        ggml_init_params params{graph_arena_bytes, nullptr, true};
        ctx_.reset(ggml_init(params));
        if (ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize NeuTTS codec graph context");
        }
        const auto & config = runtime_->assets().codec;
        const auto & weights = runtime_->weights();
        core::ModuleBuildContext ctx{ctx_.get(), "neutts.codec", runtime_->backend_type()};
        ...
        if (gallocr_ == nullptr || !ggml_gallocr_reserve(gallocr_, graph_) ||
            ...
            throw std::runtime_error("failed to allocate NeuTTS codec graph");
        }
        ...
        debug::timing_log_scalar(
            "neutts.codec.graph.build_ms",
            engine::debug::elapsed_ms(build_start, Clock::now()));
        debug::trace_log_scalar("neutts.codec.frames", frames_);
    }
    ...
    NeuTTSCodecHead run(const std::vector<int32_t> & codes) {
        const auto & config = runtime_->assets().codec;
        if (static_cast<int64_t>(codes.size()) != frames_) {
            throw std::runtime_error("NeuTTS codec code count mismatch");
        }
        const auto levels = decode_neutts_fsq_levels(codes, config.quantization_levels);
        ...
        debug::timing_log_scalar(
            "neutts.codec.graph.compute_ms",
            engine::debug::elapsed_ms(compute_start, Clock::now()));
        if (status != GGML_STATUS_SUCCESS) {
            throw std::runtime_error("NeuTTS codec graph compute failed");
        }
        NeuTTSCodecHead out;
        out.frames = frames_;
...
NeuTTSCodecDecoderWeights load_neutts_codec_decoder_weights(
    ...
    const auto & config = assets.codec;
    const auto & source = *assets.codec_weights;
    NeuTTSCodecDecoderWeights weights;
    weights.store = std::make_shared<core::BackendWeightStore>(
        backend,
        backend_type,
        "neutts.codec.weights",
        weight_context_bytes);
    ...
    for (int64_t layer = 0; layer < config.layers; ++layer) {
        weights.transformer_layers.push_back(load_transformer_layer(
            *weights.store,
            source,
            "acoustic_decoder.layers." + std::to_string(layer),
            config,
            backend_type,
            matmul_storage_type));
    }
    weights.post_blocks.reserve(2);
    for (int64_t block = 0; block < 2; ++block) {
        weights.post_blocks.push_back(load_resnet_block(
            *weights.store,
            source,
            "acoustic_decoder.post_net." + std::to_string(block),
            config.hidden_size,
            conv_storage_type));
    }
    weights.final_norm = load_affine_norm(*weights.store, source, "acoustic_decoder.norm", config.hidden_size);
...
class NeuTTSCodecDecoderRuntime::Impl {
public:
    Impl(
        std::shared_ptr<const NeuTTSAssets> assets,
        core::ExecutionContext & execution,
    ...
    NeuTTSCodecHead decode_head(const std::vector<int32_t> & codes) {
        if (codes.empty()) {
            throw std::runtime_error("NeuTTS codec cannot decode an empty code sequence");
        }
    ...
    std::vector<float> decode_audio(const std::vector<int32_t> & codes) {
        ...
        debug::timing_log_scalar(
            "neutts.codec.head.total_ms",
            engine::debug::elapsed_ms(timing_start, Clock::now()));
        const auto & config = weights_->assets().codec;
...
NeuTTSCodecDecoderRuntime::NeuTTSCodecDecoderRuntime(
    std::shared_ptr<const NeuTTSAssets> assets,
    core::ExecutionContext & execution,
    size_t graph_arena_bytes,
    size_t weight_context_bytes,
...
NeuTTSCodecDecoderRuntime::~NeuTTSCodecDecoderRuntime() = default;

NeuTTSCodecDecoderRuntime::NeuTTSCodecDecoderRuntime(NeuTTSCodecDecoderRuntime &&) noexcept = default;

NeuTTSCodecDecoderRuntime & NeuTTSCodecDecoderRuntime::operator=(
    NeuTTSCodecDecoderRuntime &&) noexcept = default;

NeuTTSCodecHead NeuTTSCodecDecoderRuntime::decode_head(const std::vector<int32_t> & codes) {
    return impl_->decode_head(codes);
}

std::vector<float> NeuTTSCodecDecoderRuntime::decode_audio(const std::vector<int32_t> & codes) {
...
std::vector<float> decode_neutts_fsq_levels(
    ...
    if (levels.empty()) {
        throw std::runtime_error("NeuTTS FSQ levels are empty");
    }
    ...
    for (size_t frame = 0; frame < codes.size(); ++frame) {
        ...
        if (code < 0 || code >= codebook_size) {
            throw std::runtime_error("NeuTTS FSQ code is outside the codebook");
        }
        int64_t basis = 1;
