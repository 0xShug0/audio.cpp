...
private:
    ...
    CachedGraph & graph_for(const std::vector<int64_t> & input_shape) {
        const auto found = graphs_.find(input_shape);
    ...
    int threads_ = 1;
    std::map<std::vector<int64_t>, std::unique_ptr<CachedGraph>> graphs_;
};
...
private:
    ...
    CachedGraph & graph_for(
        ...
        const auto found = graphs_.find(key);
        if (found != graphs_.end()) {
            return *found->second;
        }
    ...
    CachedGraph & prepare(
        ...
        tensor_writer.flush();
        auto inserted = graphs_.emplace(key, std::move(graph));
