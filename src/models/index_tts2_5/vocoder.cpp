...
engine::modules::BigVganVocoderConfig parse_bigvgan_config(
    ...
    if (config.sampling_rate != assets.config.s2mel.sample_rate ||
        ...
        throw std::runtime_error("IndexTTS2.5 BigVGAN config does not match S2Mel mel config");
    }
    return config;
}
...
IndexTTS25BigVganVocoder::IndexTTS25BigVganVocoder(
    std::shared_ptr<const IndexTTS25Assets> assets,
    core::BackendConfig backend,
    engine::assets::TensorStorageType weight_storage_type)
    : assets_(std::move(assets)) {
    if (assets_ == nullptr) {
        throw std::runtime_error("IndexTTS2.5 BigVGAN vocoder requires assets");
    }
...
IndexTTS25VocoderOutput IndexTTS25BigVganVocoder::synthesize(
...
void IndexTTS25BigVganVocoder::release_runtime_graph() {
