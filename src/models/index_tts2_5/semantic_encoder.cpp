...
core::TensorValue wav2vec2bert_attention(
    ...
    const core::TensorValue & distance_ids,
    const IndexTTS25Wav2Vec2BertAttentionWeights & weights) {
...
core::TensorValue wav2vec2bert_conv(
    ...
    const core::TensorValue & input,
    const core::TensorValue & keep_mask,
    const IndexTTS25Wav2Vec2BertConvWeights & weights) {
...
core::TensorValue wav2vec2bert_layer(
    ...
    const core::TensorValue & attention_mask,
    const core::TensorValue & distance_ids,
    const IndexTTS25Wav2Vec2BertLayerWeights & weights) {
...
IndexTTS25Wav2Vec2BertAttentionWeights load_attention(
    engine::core::BackendWeightStore & store,
    const engine::assets::TensorSource & source,
    const std::string & prefix,
    engine::assets::TensorStorageType storage_type) {
    IndexTTS25Wav2Vec2BertAttentionWeights weights;
...
IndexTTS25Wav2Vec2BertConvWeights load_conv_module(
    engine::core::BackendWeightStore & store,
    const engine::assets::TensorSource & source,
    const std::string & prefix,
    engine::assets::TensorStorageType storage_type) {
    IndexTTS25Wav2Vec2BertConvWeights weights;
...
IndexTTS25Wav2Vec2BertLayerWeights load_layer(
    engine::core::BackendWeightStore & store,
    const engine::assets::TensorSource & source,
    int64_t layer_index,
    ...
    IndexTTS25Wav2Vec2BertLayerWeights layer;
...
std::vector<float> make_attention_mask(const std::vector<int32_t> & keep_mask, int64_t frames) {
    if (static_cast<int64_t>(keep_mask.size()) != frames) {
        throw std::runtime_error("IndexTTS2.5 Wav2Vec2-BERT attention mask length mismatch");
    }
...
class IndexTTS25Wav2Vec2BertRuntime::Graph {
public:
    Graph(
        engine::core::ExecutionContext & execution,
        std::shared_ptr<const IndexTTS25Wav2Vec2BertWeights> weights,
        int64_t frames,
        ...
        : execution_(execution),
          ...
          frames_(frames) {
        if (frames_ <= 0) {
            throw std::runtime_error("IndexTTS2.5 Wav2Vec2-BERT graph requires positive frame count");
        }
        if (weights_ == nullptr) {
            throw std::runtime_error("IndexTTS2.5 Wav2Vec2-BERT graph requires weights");
        }
        ...
        if (ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 Wav2Vec2-BERT graph context");
        }
        ...
        if (input_ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 Wav2Vec2-BERT input context");
        }
        core::ModuleBuildContext ctx{ctx_.get(), "index_tts2_5.wav2vec2bert", execution_.backend_type()};
        core::ModuleBuildContext input_ctx{
            input_ctx_.get(),
            "index_tts2_5.wav2vec2bert.inputs",
        ...
        if (input_buffer_ == nullptr) {
            throw std::runtime_error("failed to allocate IndexTTS2.5 Wav2Vec2-BERT input buffer");
        }
        ...
        if (gallocr_ == nullptr ||
            ...
            clear_graph();
            throw std::runtime_error("failed to allocate IndexTTS2.5 Wav2Vec2-BERT graph");
        }
        ...
        debug::timing_log_scalar(
            "index_tts2_5.wav2vec2bert.graph.build_ms",
            engine::debug::elapsed_ms(build_start, Clock::now()));
        debug::trace_log_scalar("index_tts2_5.wav2vec2bert.frames", frames_);
    }
    ...
    IndexTTS25SemanticEmbedding run(const IndexTTS25SemanticFeatureOutput & features) {
        if (features.frames <= 0 || features.frames > frames_ || features.dims != kFeatureDim) {
            throw std::runtime_error("IndexTTS2.5 Wav2Vec2-BERT feature shape does not match prepared graph");
        }
        if (static_cast<int64_t>(features.values.size()) != features.frames * kFeatureDim) {
            throw std::runtime_error("IndexTTS2.5 Wav2Vec2-BERT feature value count mismatch");
        }
        if (static_cast<int64_t>(features.attention_mask.size()) != features.frames) {
            throw std::runtime_error("IndexTTS2.5 Wav2Vec2-BERT feature attention mask count mismatch");
        }
        ...
        debug::timing_log_scalar(
            "index_tts2_5.wav2vec2bert.input_upload_ms",
        ...
        debug::timing_log_scalar(
            "index_tts2_5.wav2vec2bert.graph.compute_ms",
            engine::debug::elapsed_ms(timing_start, Clock::now()));
        if (status != GGML_STATUS_SUCCESS) {
            throw std::runtime_error("IndexTTS2.5 Wav2Vec2-BERT graph compute failed");
        }
        IndexTTS25SemanticEmbedding out;
        ...
        debug::timing_log_scalar(
            "index_tts2_5.wav2vec2bert.output_read_ms",
            engine::debug::elapsed_ms(timing_start, Clock::now()));
        return out;
    }
...
std::shared_ptr<const IndexTTS25Wav2Vec2BertWeights> load_index_tts2_5_wav2vec2bert_weights(
    ...
    if (assets.wav2vec2bert_weights == nullptr || assets.wav2vec2bert_stats == nullptr) {
        throw std::runtime_error("IndexTTS2.5 Wav2Vec2-BERT requires model and stats tensor sources");
    }
    auto weights = std::make_shared<IndexTTS25Wav2Vec2BertWeights>();
    weights->store = std::make_shared<engine::core::BackendWeightStore>(
        backend,
        backend_type,
        "index_tts2_5.wav2vec2bert.weights",
...
IndexTTS25Wav2Vec2BertRuntime::IndexTTS25Wav2Vec2BertRuntime(
    ...
    if (assets_ == nullptr) {
        throw std::runtime_error("IndexTTS2.5 Wav2Vec2-BERT runtime requires assets");
    }
    if (graph_arena_bytes_ == 0) {
        throw std::runtime_error("IndexTTS2.5 Wav2Vec2-BERT graph arena must be non-zero");
    }
    weights_ = load_index_tts2_5_wav2vec2bert_weights(
        *assets_,
        execution.backend(),
...
IndexTTS25Wav2Vec2BertRuntime::~IndexTTS25Wav2Vec2BertRuntime() = default;

void IndexTTS25Wav2Vec2BertRuntime::prepare(int64_t frames) {
    if (execution_ == nullptr) {
        throw std::runtime_error("IndexTTS2.5 Wav2Vec2-BERT runtime execution context is missing");
    }
    if (frames <= 0) {
        throw std::runtime_error("IndexTTS2.5 Wav2Vec2-BERT prepare requires positive frames");
    }
...
IndexTTS25SemanticEmbedding IndexTTS25Wav2Vec2BertRuntime::encode(const IndexTTS25SemanticFeatureOutput & features) {
    if (graph_ == nullptr || graph_->frames() < features.frames) {
        throw std::runtime_error("IndexTTS2.5 Wav2Vec2-BERT graph was not prepared for this reference length");
    }
    return graph_->run(features);
}

void IndexTTS25Wav2Vec2BertRuntime::release_graph() {
    graph_.reset();
}
