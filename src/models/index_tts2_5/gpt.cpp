...
constexpr int64_t kTextTokens = 60510;
...
// spk_cond_mode="campplus": the projected 192-dim CAMPPlus embedding forms a
// single speaker token, followed by two all-zero tokens.
constexpr int64_t kCampplusStyleDim = 192;
constexpr int64_t kConditionTokens = 3;
...
core::TensorValue build_biased_gpt_projection(
    ...
    if (out_features % 4 != 0) {
        throw std::runtime_error("IndexTTS2.5 GPT fast projection requires output features divisible by 4");
    }
    ...
    if (!weights.bias.has_value()) {
        throw std::runtime_error("IndexTTS2.5 GPT linear bias is missing");
    }
...
core::TensorValue glu_axis(
    ...
    if (axis < 0 || axis >= static_cast<int>(input.shape.rank) || input.shape.dims[static_cast<size_t>(axis)] % 2 != 0) {
        throw std::runtime_error("IndexTTS2.5 GLU axis shape mismatch");
    }
...
core::TensorValue condition_subsample(
    core::ModuleBuildContext & ctx,
    const core::TensorValue & input,
    const IndexTTS25GptConditionEncoderWeights & weights) {
...
core::TensorValue condition_rel_attention(
    ...
    const IndexTTS25GptConditionLayerWeights & weights,
    int64_t heads) {
...
core::TensorValue perceiver_attention(
    ...
    const IndexTTS25PerceiverAttentionWeights & weights,
    int64_t dim,
    int64_t heads,
    int64_t inner) {
...
core::TensorValue perceiver_ff(
    core::ModuleBuildContext & ctx,
    const core::TensorValue & input,
    const IndexTTS25PerceiverFeedForwardWeights & weights,
    int64_t dim,
    int64_t ff_in) {
...
core::TensorValue perceiver(
    core::ModuleBuildContext & ctx,
    const core::TensorValue & input,
    const IndexTTS25PerceiverWeights & weights,
...
core::TensorValue condition_encoder(
    core::ModuleBuildContext & ctx,
    const core::TensorValue & input,
    const IndexTTS25GptConditionEncoderWeights & encoder,
    const IndexTTS25PerceiverWeights & perceiver_weights,
...
IndexTTS25GptConditionLayerWeights load_condition_layer(
    engine::core::BackendWeightStore & store,
    const engine::assets::TensorSource & source,
    const std::string & prefix,
    ...
    IndexTTS25GptConditionLayerWeights layer;
...
IndexTTS25GptConditionEncoderWeights load_condition_encoder(
    engine::core::BackendWeightStore & store,
    const engine::assets::TensorSource & source,
    const std::string & prefix,
    int64_t layers,
    ...
    engine::assets::TensorStorageType matmul_storage_type,
    engine::assets::TensorStorageType conv_storage_type) {
    IndexTTS25GptConditionEncoderWeights encoder;
...
IndexTTS25PerceiverLayerWeights load_perceiver_layer(
    engine::core::BackendWeightStore & store,
    const engine::assets::TensorSource & source,
    const std::string & prefix,
    int64_t dim,
    ...
    int64_t ff_in,
    engine::assets::TensorStorageType storage_type) {
    IndexTTS25PerceiverLayerWeights layer;
...
IndexTTS25PerceiverWeights load_perceiver(
    engine::core::BackendWeightStore & store,
    const engine::assets::TensorSource & source,
    const std::string & prefix,
    int64_t latents,
    ...
    int64_t ff_in,
    engine::assets::TensorStorageType storage_type) {
    IndexTTS25PerceiverWeights weights;
...
IndexTTS25Gpt2LayerWeights load_gpt2_layer(
    engine::core::BackendWeightStore & store,
    const engine::assets::TensorSource & source,
    int64_t layer_index,
    engine::assets::TensorStorageType storage_type) {
    const std::string prefix = "gpt.h." + std::to_string(layer_index);
    IndexTTS25Gpt2LayerWeights layer;
...
core::TensorValue gpt_mlp(
    core::ModuleBuildContext & ctx,
    const core::TensorValue & input,
    const IndexTTS25Gpt2LayerWeights & weights) {
...
Gpt2LayerOutput gpt2_layer_cached_tail(
    ...
    if (cache_key.shape.dims[0] != input.shape.dims[0] ||
        ...
        throw std::runtime_error("IndexTTS2.5 GPT cached layer batch cache shape mismatch");
    }
    auto normed = modules::LayerNormModule({kModelDim, 1.0e-5F, true, true}).build(ctx, input, weights.attn_norm);
    auto qkv = build_biased_gpt_projection(ctx, normed, kModelDim, 3 * kModelDim, weights.qkv);
    auto q = modules::SliceModule({2, 0, kModelDim}).build(ctx, qkv);
...
struct IndexTTS25SamplerWorkspace {
...
void apply_repetition_penalty(
    ...
    float penalty,
    IndexTTS25SamplerWorkspace & workspace) {
    if (penalty == 1.0F) {
        return;
    }
    if (!(penalty > 0.0F)) {
        throw std::runtime_error("IndexTTS2.5 GPT repetition_penalty must be positive");
    }
...
void index_tts2_5_log_probs(
    ...
    IndexTTS25SamplerWorkspace & workspace) {
    if (!(temperature > 0.0F)) {
        throw std::runtime_error("IndexTTS2.5 GPT temperature must be positive");
    }
    ...
    if (!(total > 0.0F)) {
        throw std::runtime_error("IndexTTS2.5 GPT sampler invalid logit mass");
    }
    ...
    if (top_k > 0 && static_cast<size_t>(top_k) < scores.size()) {
        ...
        if (!std::isfinite(max_score)) {
            throw std::runtime_error("IndexTTS2.5 GPT sampler has no finite score");
        }
    } else {
    ...
    if (top_p > 0.0F && top_p < 1.0F) {
        ...
        if (!(total > 0.0F)) {
            throw std::runtime_error("IndexTTS2.5 GPT sampler invalid top-p mass");
        }
...
void sample_index_tts2_5_indices(
    ...
    if (!std::isfinite(max_score)) {
        throw std::runtime_error("IndexTTS2.5 GPT sampler has no finite beam score");
    }
    ranked.clear();
    ...
    if (ranked.empty()) {
        throw std::runtime_error("IndexTTS2.5 GPT sampler failed to select beam candidates");
    }
...
std::shared_ptr<const IndexTTS25GptWeights> load_index_tts2_5_gpt_weights(
    ...
    if (assets.gpt_weights == nullptr) {
        throw std::runtime_error("IndexTTS2.5 GPT requires tensor source");
    }
    auto weights = std::make_shared<IndexTTS25GptWeights>();
    weights->store = std::make_shared<engine::core::BackendWeightStore>(
        backend,
        backend_type,
        "index_tts2_5.gpt.weights",
    ...
    weights->lang_embedding = weights->store->load_tensor(
        ...
        matmul_storage_type,
        {kIndexTTS25LangEmbeddingRows, kModelDim});
...
class IndexTTS25GptRuntime::ConditioningGraph {
public:
    ConditioningGraph(
        core::ExecutionContext & execution,
        std::shared_ptr<const IndexTTS25GptWeights> weights,
        int64_t frames,
        ...
        : execution_(execution),
          ...
          frames_(frames) {
        if (frames_ <= 0) {
            throw std::runtime_error("IndexTTS2.5 GPT conditioning graph requires positive frame count");
        }
        if (weights_ == nullptr) {
            throw std::runtime_error("IndexTTS2.5 GPT conditioning graph requires weights");
        }
        ...
        if (ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 GPT conditioning graph context");
        }
        ...
        if (input_ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 GPT conditioning input context");
        }
        core::ModuleBuildContext ctx{ctx_.get(), "index_tts2_5.gpt.emo_condition", execution_.backend_type()};
        core::ModuleBuildContext input_ctx{
            input_ctx_.get(),
            "index_tts2_5.gpt.emo_condition.inputs",
        ...
        if (input_buffer_ == nullptr) {
            throw std::runtime_error("failed to allocate IndexTTS2.5 GPT conditioning input buffer");
        }
        ...
        if (gallocr_ == nullptr ||
            ...
            clear_graph();
            throw std::runtime_error("failed to allocate IndexTTS2.5 GPT conditioning graph");
        }
        debug::timing_log_scalar(
            "index_tts2_5.gpt.emo_condition.graph.build_ms",
            engine::debug::elapsed_ms(build_start, Clock::now()));
        debug::trace_log_scalar("index_tts2_5.gpt.emo_condition.frames", frames_);
    }
    ...
    IndexTTS25GptLatent run(const std::vector<float> & semantic_btc) {
        if (static_cast<int64_t>(semantic_btc.size()) != frames_ * kSemanticHidden) {
            throw std::runtime_error("IndexTTS2.5 GPT conditioning input value count mismatch");
        }
        auto timing_start = Clock::now();
        ggml_backend_tensor_set(semantic_, semantic_btc.data(), 0, semantic_btc.size() * sizeof(float));
        debug::timing_log_scalar(
            "index_tts2_5.gpt.emo_condition.input_upload_ms",
        ...
        debug::timing_log_scalar(
            "index_tts2_5.gpt.emo_condition.graph.compute_ms",
            engine::debug::elapsed_ms(timing_start, Clock::now()));
        if (status != GGML_STATUS_SUCCESS) {
            throw std::runtime_error("IndexTTS2.5 GPT conditioning graph compute failed");
        }

        IndexTTS25GptLatent output;
        ...
        debug::timing_log_scalar(
            "index_tts2_5.gpt.emo_condition.output_read_ms",
            engine::debug::elapsed_ms(timing_start, Clock::now()));
        return output;
    }
...
class IndexTTS25GptRuntime::EmotionVectorGraph {
public:
    EmotionVectorGraph(
        core::ExecutionContext & execution,
        std::shared_ptr<const IndexTTS25GptWeights> weights,
        ...
        : execution_(execution),
          weights_(std::move(weights)) {
        if (weights_ == nullptr) {
            throw std::runtime_error("IndexTTS2.5 GPT emotion vector graph requires weights");
        }
        const auto build_start = Clock::now();
        ggml_init_params params{graph_arena_bytes, nullptr, true};
        ctx_.reset(ggml_init(params));
        if (ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 GPT emotion vector graph context");
        }
        ggml_init_params input_params{16ull * 1024ull * 1024ull, nullptr, true};
        input_ctx_.reset(ggml_init(input_params));
        if (input_ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 GPT emotion vector input context");
        }
        core::ModuleBuildContext ctx{ctx_.get(), "index_tts2_5.gpt.emotion_vector", execution_.backend_type()};
        core::ModuleBuildContext input_ctx{
            input_ctx_.get(),
            "index_tts2_5.gpt.emotion_vector.inputs",
            execution_.backend_type()};
        ...
        input_buffer_ = ggml_backend_alloc_ctx_tensors(input_ctx_.get(), execution_.backend());
        if (input_buffer_ == nullptr) {
            throw std::runtime_error("failed to allocate IndexTTS2.5 GPT emotion vector input buffer");
        }
        gallocr_ = ggml_gallocr_new(ggml_backend_get_default_buffer_type(execution_.backend()));
        if (gallocr_ == nullptr ||
            !ggml_gallocr_reserve(gallocr_, graph_) ||
            !ggml_gallocr_alloc_graph(gallocr_, graph_)) {
            clear_graph();
            throw std::runtime_error("failed to allocate IndexTTS2.5 GPT emotion vector graph");
        }
        debug::timing_log_scalar("index_tts2_5.gpt.emotion_vector.graph.build_ms", engine::debug::elapsed_ms(build_start, Clock::now()));
    }
    ...
    std::vector<float> run(const IndexTTS25GptLatent & emotion_conditioning) {
        if (emotion_conditioning.frames != 1 ||
            ...
            throw std::runtime_error("IndexTTS2.5 GPT emotion vector input shape mismatch");
        }
        ...
        ggml_backend_synchronize(execution_.backend());
        if (status != GGML_STATUS_SUCCESS) {
            throw std::runtime_error("IndexTTS2.5 GPT emotion vector graph compute failed");
        }
...
class IndexTTS25GptRuntime::PrefillGraph {
public:
    PrefillGraph(
        core::ExecutionContext & execution,
        std::shared_ptr<const IndexTTS25GptWeights> weights,
        ...
        if (weights_ == nullptr || text_tokens_ < 0) {
            throw std::runtime_error("IndexTTS2.5 GPT prefill graph requires weights and non-negative text tokens");
        }
        const auto build_start = Clock::now();
        ggml_init_params params{graph_arena_bytes, nullptr, true};
        ctx_.reset(ggml_init(params));
        if (ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 GPT prefill graph context");
        }
        ggml_init_params input_params{16ull * 1024ull * 1024ull, nullptr, true};
        input_ctx_.reset(ggml_init(input_params));
        if (input_ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 GPT prefill input context");
        }
        ...
        if (output_ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 GPT prefill output context");
        }
        core::ModuleBuildContext ctx{ctx_.get(), "index_tts2_5.gpt.prefill", execution_.backend_type()};
        core::ModuleBuildContext input_ctx{
            input_ctx_.get(),
            "index_tts2_5.gpt.prefill.inputs",
            execution_.backend_type()};
        core::ModuleBuildContext output_ctx{
            output_ctx_.get(),
            "index_tts2_5.gpt.prefill.outputs",
            execution_.backend_type()};
        ...
        // campplus conditioning prefix (model_v2.py inference_speech):
        ...
        auto conds = modules::AddModule{}.build(ctx, speaker_token, emo_vec);
        auto zero_token = modules::RepeatModule({core::TensorShape::from_dims({1, kConditionTokens - 1, kModelDim})})
        ...
        auto lang = modules::EmbeddingModule({kIndexTTS25LangEmbeddingRows, kModelDim}).build(ctx, lang_id, weights_->lang_embedding);
        ...
        input_buffer_ = ggml_backend_alloc_ctx_tensors(input_ctx_.get(), execution_.backend());
        if (input_buffer_ == nullptr) {
            throw std::runtime_error("failed to allocate IndexTTS2.5 GPT prefill input buffer");
        }
        output_buffer_ = ggml_backend_alloc_ctx_tensors(output_ctx_.get(), execution_.backend());
        if (output_buffer_ == nullptr) {
            clear_graph();
            throw std::runtime_error("failed to allocate IndexTTS2.5 GPT prefill output buffer");
        }
        gallocr_ = ggml_gallocr_new(ggml_backend_get_default_buffer_type(execution_.backend()));
        if (gallocr_ == nullptr ||
            !ggml_gallocr_reserve(gallocr_, graph_) ||
            !ggml_gallocr_alloc_graph(gallocr_, graph_)) {
            clear_graph();
            throw std::runtime_error("failed to allocate IndexTTS2.5 GPT prefill graph");
        }
        const int32_t start_mel = kStartMelToken;
        ggml_backend_tensor_set(start_mel_id_, &start_mel, 0, sizeof(int32_t));
        debug::timing_log_scalar("index_tts2_5.gpt.prefill.graph.build_ms", engine::debug::elapsed_ms(build_start, Clock::now()));
        debug::trace_log_scalar("index_tts2_5.gpt.prefill.prompt_steps", prompt_steps_);
    }
    ...
    GptPrefillOutput run(
        ...
        if (static_cast<int64_t>(speaker_style.size()) != kCampplusStyleDim ||
            ...
            throw std::runtime_error("IndexTTS2.5 GPT prefill input shape mismatch");
        }
        if (lang_id < 0 || lang_id >= kIndexTTS25LangEmbeddingRows) {
            throw std::runtime_error("IndexTTS2.5 GPT prefill lang id is out of range");
        }
        ...
        debug::timing_log_scalar("index_tts2_5.gpt.prefill.input_upload_ms", engine::debug::elapsed_ms(timing_start, Clock::now()));
        core::set_backend_threads(execution_.backend(), execution_.config().threads);
        timing_start = Clock::now();
        const ggml_status status = core::compute_backend_graph(execution_.backend(), graph_);
        ggml_backend_synchronize(execution_.backend());
        debug::timing_log_scalar("index_tts2_5.gpt.prefill.graph.compute_ms", engine::debug::elapsed_ms(timing_start, Clock::now()));
        if (status != GGML_STATUS_SUCCESS) {
            throw std::runtime_error("IndexTTS2.5 GPT prefill graph compute failed");
        }
...
class IndexTTS25GptRuntime::DecodeGraph {
public:
    struct StepOutput {
        std::vector<float> logits;
    };
    ...
    DecodeGraph(
        ...
        if (weights_ == nullptr || cache_steps_ <= 0 || beam_count_ <= 0) {
            throw std::runtime_error("IndexTTS2.5 GPT decode graph requires weights and cache steps");
        }
        const auto build_start = Clock::now();
        ggml_init_params params{graph_arena_bytes, nullptr, true};
        ctx_.reset(ggml_init(params));
        if (ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 GPT decode graph context");
        }
        ...
        if (state_ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 GPT decode state context");
        }
        core::ModuleBuildContext ctx{ctx_.get(), "index_tts2_5.gpt.decode", execution_.backend_type()};
        core::ModuleBuildContext state_ctx{
            state_ctx_.get(),
            "index_tts2_5.gpt.decode.state",
            execution_.backend_type()};
        ...
        if (state_buffer_ == nullptr) {
            throw std::runtime_error("failed to allocate IndexTTS2.5 GPT decode state buffer");
        }
        debug::trace_log_scalar(
            "index_tts2_5.gpt.decode.state_buffer_mib",
        ...
        if (gallocr_ == nullptr ||
            ...
            clear_graph();
            throw std::runtime_error("failed to allocate IndexTTS2.5 GPT decode graph");
        }
        ...
        debug::timing_log_scalar("index_tts2_5.gpt.decode.graph.build_ms", engine::debug::elapsed_ms(build_start, Clock::now()));
        debug::trace_log_scalar("index_tts2_5.gpt.decode.cache_steps", cache_steps_);
        debug::trace_log_scalar("index_tts2_5.gpt.decode.beam_batch", beam_count_);
    }
    ...
    void initialize_beam_slot(int64_t slot, const runtime::TransformerKVState & state) {
        if (slot < 0 || slot >= beam_slots_) {
            throw std::runtime_error("IndexTTS2.5 GPT beam slot is out of range");
        }
        if (state.layers.size() != weights_->gpt_layers.size()) {
            throw std::runtime_error("IndexTTS2.5 GPT beam state layer count mismatch");
        }
        for (size_t layer = 0; layer < state.layers.size(); ++layer) {
            ...
            if (!state.layers.empty() && layer_state.valid_steps != state.layers.front().valid_steps) {
                throw std::runtime_error("IndexTTS2.5 GPT beam state valid step mismatch");
            }
    ...
    BatchOutput run_batch_from_beams(
        ...
        if (active == 0 || child_slots.size() != active || tokens.size() != active) {
            throw std::runtime_error("IndexTTS2.5 GPT batched decode input shape mismatch");
        }
        if (active > static_cast<size_t>(beam_count_)) {
            throw std::runtime_error("IndexTTS2.5 GPT batched decode exceeds beam batch");
        }
        if (valid_steps >= cache_steps_) {
            throw std::runtime_error("IndexTTS2.5 GPT decode cache exhausted");
        }
        const int64_t child_bank = child_slots.front() / beam_count_;
        if (child_bank < 0 || child_bank > 1) {
            throw std::runtime_error("IndexTTS2.5 GPT child beam bank is out of range");
        }
        for (size_t row = 0; row < active; ++row) {
            if (parent_slots[row] < 0 || parent_slots[row] >= beam_slots_ ||
                ...
                throw std::runtime_error("IndexTTS2.5 GPT beam slot layout mismatch");
            }
            for (size_t layer = 0; layer < weights_->gpt_layers.size(); ++layer) {
        ...
        if (!ggml_gallocr_alloc_graph(gallocr_, graph.graph)) {
            throw std::runtime_error("failed to allocate IndexTTS2.5 GPT decode bank graph");
        }
        core::set_backend_threads(execution_.backend(), execution_.config().threads);
        const ggml_status status = core::compute_backend_graph(execution_.backend(), graph.graph);
        ggml_backend_synchronize(execution_.backend());
        if (status != GGML_STATUS_SUCCESS) {
            throw std::runtime_error("IndexTTS2.5 GPT decode graph compute failed");
        }
...
private:
    ...
    void copy_beam_prefix(int64_t parent_slot, int64_t child_slot, int64_t valid_steps, size_t layer) {
        ...
        if (valid_steps > cache_steps_) {
            throw std::runtime_error("IndexTTS2.5 GPT beam prefix copy exceeds cache capacity");
        }
...
IndexTTS25GptRuntime::IndexTTS25GptRuntime(
    ...
    if (assets_ == nullptr) {
        throw std::runtime_error("IndexTTS2.5 GPT runtime requires assets");
    }
    if (graph_arena_bytes_ == 0) {
        throw std::runtime_error("IndexTTS2.5 GPT graph arena must be non-zero");
    }
    weights_ = load_index_tts2_5_gpt_weights(
        *assets_,
        execution.backend(),
...
IndexTTS25GptRuntime::~IndexTTS25GptRuntime() = default;

void IndexTTS25GptRuntime::prepare_emotion_conditioning(int64_t frames) {
    if (execution_ == nullptr) {
        throw std::runtime_error("IndexTTS2.5 GPT runtime execution context is missing");
    }
    if (frames <= 0) {
        throw std::runtime_error("IndexTTS2.5 GPT emotion conditioning prepare requires positive frames");
    }
    if (emotion_conditioning_graph_ != nullptr && emotion_conditioning_graph_->frames() == frames) {
        return;
    }
    emotion_conditioning_graph_.reset();
    emotion_conditioning_graph_ = std::make_unique<ConditioningGraph>(*execution_, weights_, frames, graph_arena_bytes_);
}

IndexTTS25GptLatent IndexTTS25GptRuntime::emotion_conditioning(const std::vector<float> & semantic_btc, int64_t frames) {
    if (emotion_conditioning_graph_ == nullptr || emotion_conditioning_graph_->frames() != frames) {
        throw std::runtime_error("IndexTTS2.5 GPT emotion conditioning graph was not prepared for this reference length");
    }
    return emotion_conditioning_graph_->run(semantic_btc);
}

void IndexTTS25GptRuntime::prepare_generation(int64_t text_tokens, int64_t max_mel_tokens, int64_t num_beams) {
    if (execution_ == nullptr) {
        throw std::runtime_error("IndexTTS2.5 GPT runtime execution context is missing");
    }
    if (text_tokens < 0 || max_mel_tokens <= 0) {
        throw std::runtime_error("IndexTTS2.5 GPT generation prepare requires non-negative text tokens and positive mel tokens");
    }
    if (num_beams != 1) {
        debug::trace_log_scalar("index_tts2_5.gpt.generation.num_beams", num_beams);
    }
...
std::vector<float> IndexTTS25GptRuntime::project_emotion_vector(const IndexTTS25GptLatent & emotion_conditioning) {
...
std::vector<float> IndexTTS25GptRuntime::merge_emotion_vector(
...
IndexTTS25GptGeneration IndexTTS25GptRuntime::generate_speech(const IndexTTS25GptGenerationRequest & request) {
    if (request.text_tokens.empty()) {
        throw std::runtime_error("IndexTTS2.5 GPT generation requires text tokens");
    }
    const auto text_tokens = align_index_tts2_5_gpt_text_tokens(request.text_tokens);
    if (static_cast<int64_t>(request.speaker_style.size()) != kCampplusStyleDim) {
        throw std::runtime_error("IndexTTS2.5 GPT generation speaker style shape mismatch");
    }
    if (request.lang_id < 0 || request.lang_id >= kIndexTTS25LangEmbeddingRows) {
        throw std::runtime_error("IndexTTS2.5 GPT generation lang id is out of range");
    }
    ...
    if (static_cast<int64_t>(emotion_vector.size()) != kModelDim) {
        throw std::runtime_error("IndexTTS2.5 GPT generation emotion vector shape mismatch");
    }
    prepare_generation(
        static_cast<int64_t>(text_tokens.size()),
        request.max_mel_tokens,
        request.num_beams);
    auto prefill = prefill_graph_->run(request.speaker_style, emotion_vector, request.lang_id, text_tokens);
    const auto sampling_policy = engine::sampling::resolve_torch_cuda_sampling_policy(
        execution_->backend_type(),
        execution_->config().device,
        "index_tts2_5.gpt.cuda_sampling_policy",
        "IndexTTS2.5",
    ...
    IndexTTS25SamplerWorkspace sampler_workspace;
    ...
    for (int step = 0; step < request.max_mel_tokens && !beams.empty(); ++step) {
        ...
        if (!next_beams.empty()) {
            ...
            if (batch_out.steps.size() != next_beams.size()) {
                throw std::runtime_error("IndexTTS2.5 GPT batched decode output size mismatch");
            }
            ...
            if (!first_decode_timing_logged) {
                debug::timing_log_scalar("index_tts2_5.gpt.decode.first_run_ms", run_ms);
    ...
    debug::timing_log_scalar("index_tts2_5.gpt.sampling_ms", sampling_ms);
    debug::timing_log_scalar("index_tts2_5.gpt.decode.run_ms", decode_run_ms);
    if (!beam_search_done) {
        for (auto & beam : beams) {
    ...
    if (completed.empty()) {
        throw std::runtime_error("IndexTTS2.5 GPT generation produced no beam candidates");
    }
    const auto best = std::max_element(completed.begin(), completed.end(), [&](const Beam & lhs, const Beam & rhs) {
        return normalized_score(lhs) < normalized_score(rhs);
    });
    IndexTTS25GptGeneration out;
    ...
    debug::trace_log_scalar("index_tts2_5.gpt.generated_code_count", static_cast<int64_t>(out.codes.size()));
    debug::trace_log_scalar("index_tts2_5.gpt.generated_stop_seen", stop_seen);
    return out;
}

void IndexTTS25GptRuntime::release_conditioning_graphs() {
    emotion_conditioning_graph_.reset();
    emotion_vector_graph_.reset();
}

void IndexTTS25GptRuntime::release_generation_graphs() {
    prefill_graph_.reset();
    decode_graph_.reset();
}
