...
const std::vector<float> & cached_mel_filterbank(const IndexTTS25S2MelConfig & config) {
...
std::vector<float> require_mono_samples(const std::vector<float> & samples, int channels) {
    if (channels <= 0) {
        throw std::runtime_error("IndexTTS2.5 audio channel count must be positive");
    }
    if (samples.empty()) {
        throw std::runtime_error("IndexTTS2.5 audio must not be empty");
    }
    if (samples.size() % static_cast<size_t>(channels) != 0) {
        throw std::runtime_error("IndexTTS2.5 audio sample count must be divisible by channel count");
    }
    if (channels == 1) {
        return samples;
    }
...
std::vector<float> resample_mono(const std::vector<float> & input, int input_sample_rate, int output_sample_rate) {
    if (input_sample_rate <= 0 || output_sample_rate <= 0) {
        throw std::runtime_error("IndexTTS2.5 resampling requires positive sample rates");
    }
...
std::vector<float> resample_mono_librosa(const std::vector<float> & input, int input_sample_rate, int output_sample_rate) {
    if (input_sample_rate <= 0 || output_sample_rate <= 0) {
        throw std::runtime_error("IndexTTS2.5 librosa-style resampling requires positive sample rates");
    }
    if (input_sample_rate == output_sample_rate || input.empty()) {
        return input;
    }
...
IndexTTS25MelOutput compute_index_tts2_5_mel_spectrogram(
    ...
    if (config.sample_rate <= 0 || config.n_fft <= 0 || config.win_length <= 0 ||
        ...
        throw std::runtime_error("IndexTTS2.5 mel spectrogram config is invalid");
    }
    if (waveform.empty()) {
        throw std::runtime_error("IndexTTS2.5 mel spectrogram requires non-empty waveform");
    }
    ...
    if (static_cast<int64_t>(filterbank.size()) != config.n_mels * freq_bins) {
        throw std::runtime_error("IndexTTS2.5 mel filterbank shape mismatch");
    }

    IndexTTS25MelOutput output;
...
IndexTTS25FbankOutput compute_index_tts2_5_campplus_fbank_16k(const std::vector<float> & waveform_16k) {
    ...
    if (static_cast<int64_t>(waveform_16k.size()) < kWindowSize) {
        throw std::runtime_error("IndexTTS2.5 CAMPPlus fbank requires at least one 25 ms frame");
    }
...
IndexTTS25SemanticFeatureOutput compute_index_tts2_5_semantic_features_16k(const std::vector<float> & waveform_16k) {
    ...
    if (static_cast<int64_t>(waveform_16k.size()) < kWindowSize) {
        throw std::runtime_error("IndexTTS2.5 semantic fbank requires at least one 25 ms frame");
    }
...
#pragma omp parallel for if(frames >= 8)
    ...
    IndexTTS25SemanticFeatureOutput output;
...
IndexTTS25PreparedReferenceAudio prepare_index_tts2_5_reference_audio(
    ...
    IndexTTS25PreparedReferenceAudio output;
    ...
    output.mel = compute_index_tts2_5_mel_spectrogram(output.waveform_22k, mel_config, threads);
    output.campplus_fbank = compute_index_tts2_5_campplus_fbank_16k(output.waveform_16k);
    output.semantic_features = compute_index_tts2_5_semantic_features_16k(output.waveform_16k);
    return output;
}
