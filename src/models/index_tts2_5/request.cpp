...
std::vector<float> parse_emotion_vector(const std::string & value) {
    ...
    while (std::getline(stream, item, ',')) {
        ...
        if (item.empty()) {
            throw std::runtime_error("IndexTTS2.5 emotion_vector contains an empty item");
        }
        size_t parsed = 0;
        const float parsed_value = std::stof(item, &parsed);
        if (parsed != item.size() || !std::isfinite(parsed_value)) {
            throw std::runtime_error("IndexTTS2.5 emotion_vector must contain finite floats");
        }
        values.push_back(parsed_value);
    }
    if (values.size() != 8) {
        throw std::runtime_error("IndexTTS2.5 emotion_vector must contain exactly 8 values");
    }
    return values;
}
...
void require_valid_audio(const runtime::AudioBuffer & audio, const char * label) {
    if (audio.sample_rate <= 0 || audio.channels <= 0 || audio.samples.empty()) {
        throw std::runtime_error(std::string("IndexTTS2.5 ") + label + " audio is empty or invalid");
    }
}
...
IndexTTS25Request parse_index_tts2_5_request(const runtime::TaskRequest & request) {
    ...
    if (const auto * speaker = speaker_audio_from_request(request)) {
        ...
        throw std::runtime_error("IndexTTS2.5 request requires --voice-ref or voice.speaker.audio");
    }

    if (const auto value = runtime::find_option(request.options, {"lang"})) {
        out.lang = normalize_index_tts2_5_lang(*value);
    }
    if (const auto value = runtime::parse_finite_float_option(request.options, {"emotion_alpha"})) {
        if (*value < 0.0F || *value > 1.0F) {
            throw std::runtime_error("IndexTTS2.5 emotion_alpha must be in [0, 1]");
        }
    ...
    if (request.voice.has_value() &&
        ...
        if (!engine::io::trim_ascii_whitespace(text).empty()) {
            if (use_emotion_text_value.has_value() && !out.use_emotion_text) {
                throw std::runtime_error("IndexTTS2.5 --emotion conflicts with use_emotion_text=false");
            }
            if (out.emotion_text.has_value() && *out.emotion_text != text) {
                throw std::runtime_error("IndexTTS2.5 --emotion conflicts with emotion_text");
            }
    ...
    if (const auto value = runtime::parse_int_option(request.options, {"interval_silence_ms"})) {
        if (*value < 0) {
            throw std::runtime_error("IndexTTS2.5 interval_silence_ms must be non-negative");
        }
    ...
    if (out.generation.top_k <= 0) {
        throw std::runtime_error("IndexTTS2.5 top_k must be positive");
    }
    if (!(out.generation.top_p > 0.0F && out.generation.top_p <= 1.0F)) {
        throw std::runtime_error("IndexTTS2.5 top_p must be in (0, 1]");
    }
    if (!(out.generation.temperature > 0.0F)) {
        throw std::runtime_error("IndexTTS2.5 temperature must be positive");
    }
