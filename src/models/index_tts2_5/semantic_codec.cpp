...
core::TensorValue vocos_backbone(
    ...
    const IndexTTS25VocosBackboneWeights & weights) {
...
core::TensorValue quantizer_in_project(
    core::ModuleBuildContext & ctx,
    const core::TensorValue & input_bct,
    const IndexTTS25SemanticCodecWeights & weights) {
...
IndexTTS25VocosConvNeXtBlockWeights load_convnext_block(
    engine::core::BackendWeightStore & store,
    const engine::assets::TensorSource & source,
    const std::string & prefix,
    engine::assets::TensorStorageType matmul_storage_type,
    engine::assets::TensorStorageType conv_storage_type) {
    IndexTTS25VocosConvNeXtBlockWeights block;
...
IndexTTS25VocosBackboneWeights load_vocos_backbone(
    engine::core::BackendWeightStore & store,
    const engine::assets::TensorSource & source,
    const std::string & prefix,
    ...
    engine::assets::TensorStorageType matmul_storage_type,
    engine::assets::TensorStorageType conv_storage_type) {
    IndexTTS25VocosBackboneWeights backbone;
...
class IndexTTS25SemanticCodecRuntime::QuantizeGraph {
public:
    QuantizeGraph(
        core::ExecutionContext & execution,
        std::shared_ptr<const IndexTTS25SemanticCodecWeights> weights,
        int64_t frames,
        ...
        : execution_(execution),
          ...
          frames_(frames) {
        if (frames_ <= 0) {
            throw std::runtime_error("IndexTTS2.5 semantic codec quantize graph requires positive frame count");
        }
        if (weights_ == nullptr) {
            throw std::runtime_error("IndexTTS2.5 semantic codec quantize graph requires weights");
        }
        ...
        if (ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 semantic codec quantize graph context");
        }
        ...
        if (input_ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 semantic codec quantize input context");
        }
        core::ModuleBuildContext ctx{ctx_.get(), "index_tts2_5.semantic_codec.quantize", execution_.backend_type()};
        core::ModuleBuildContext input_ctx{
            input_ctx_.get(),
            "index_tts2_5.semantic_codec.quantize.inputs",
        ...
        if (input_buffer_ == nullptr) {
            throw std::runtime_error("failed to allocate IndexTTS2.5 semantic codec quantize input buffer");
        }
        ...
        if (gallocr_ == nullptr ||
            ...
            clear_graph();
            throw std::runtime_error("failed to allocate IndexTTS2.5 semantic codec quantize graph");
        }
        debug::timing_log_scalar(
            "index_tts2_5.semantic_codec.quantize.graph.build_ms",
            engine::debug::elapsed_ms(build_start, Clock::now()));
        debug::trace_log_scalar("index_tts2_5.semantic_codec.quantize.frames", frames_);
    }
    ...
    IndexTTS25SemanticCodecOutput run(const IndexTTS25SemanticEmbedding & semantic) {
        if (semantic.frames != frames_ || semantic.dims != kHidden) {
            throw std::runtime_error("IndexTTS2.5 semantic codec semantic input shape does not match prepared graph");
        }
        if (static_cast<int64_t>(semantic.values.size()) != frames_ * kHidden) {
            throw std::runtime_error("IndexTTS2.5 semantic codec semantic input value count mismatch");
        }
        auto timing_start = Clock::now();
        ggml_backend_tensor_set(semantic_, semantic.values.data(), 0, semantic.values.size() * sizeof(float));
        debug::timing_log_scalar(
            "index_tts2_5.semantic_codec.quantize.input_upload_ms",
        ...
        debug::timing_log_scalar(
            "index_tts2_5.semantic_codec.quantize.graph.compute_ms",
            engine::debug::elapsed_ms(timing_start, Clock::now()));
        if (status != GGML_STATUS_SUCCESS) {
            throw std::runtime_error("IndexTTS2.5 semantic codec quantize graph compute failed");
        }

        IndexTTS25SemanticCodecOutput output;
        ...
        debug::timing_log_scalar(
            "index_tts2_5.semantic_codec.quantize.output_read_ms",
            engine::debug::elapsed_ms(timing_start, Clock::now()));
        return output;
    }
...
class IndexTTS25SemanticCodecRuntime::CodesGraph {
public:
    CodesGraph(
        core::ExecutionContext & execution,
        std::shared_ptr<const IndexTTS25SemanticCodecWeights> weights,
        ...
        : execution_(execution),
          ...
          frames_(frames) {
        if (frames_ <= 0) {
            throw std::runtime_error("IndexTTS2.5 semantic codec code graph requires positive frame count");
        }
        if (weights_ == nullptr) {
            throw std::runtime_error("IndexTTS2.5 semantic codec code graph requires weights");
        }
        ...
        ctx_.reset(ggml_init(params));
        if (ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 semantic codec code graph context");
        }
        ggml_init_params input_params{16ull * 1024ull * 1024ull, nullptr, true};
        input_ctx_.reset(ggml_init(input_params));
        if (input_ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 semantic codec code input context");
        }
        core::ModuleBuildContext ctx{ctx_.get(), "index_tts2_5.semantic_codec.codes", execution_.backend_type()};
        core::ModuleBuildContext input_ctx{
            input_ctx_.get(),
            "index_tts2_5.semantic_codec.codes.inputs",
            execution_.backend_type()};
        ...
        auto embedding = embed_codes_bct(
            ...
            core::wrap_tensor(codes_, core::TensorShape::from_dims({1, frames_}), GGML_TYPE_I32),
            *weights_);
        // EnhancedCodec.decode (codec/models.py): decoder backbone + projection,
        // then 2x nearest upsample along time and the `up` conv.
        ...
        embedding_ = core::ensure_backend_addressable_layout(ctx, x).tensor;
        ggml_set_output(embedding_);
        ...
        input_buffer_ = ggml_backend_alloc_ctx_tensors(input_ctx_.get(), execution_.backend());
        if (input_buffer_ == nullptr) {
            throw std::runtime_error("failed to allocate IndexTTS2.5 semantic codec code input buffer");
        }
        ...
        if (gallocr_ == nullptr ||
            ...
            !ggml_gallocr_alloc_graph(gallocr_, graph_)) {
            clear_graph();
            throw std::runtime_error("failed to allocate IndexTTS2.5 semantic codec code graph");
        }
        debug::timing_log_scalar(
            "index_tts2_5.semantic_codec.codes.graph.build_ms",
            engine::debug::elapsed_ms(build_start, Clock::now()));
        debug::trace_log_scalar("index_tts2_5.semantic_codec.codes.frames", frames_);
    }
    ...
    IndexTTS25SemanticCodecOutput run(const std::vector<int32_t> & codes) {
        if (static_cast<int64_t>(codes.size()) != frames_) {
            throw std::runtime_error("IndexTTS2.5 semantic codec code count does not match prepared graph");
        }
        auto timing_start = Clock::now();
        ggml_backend_tensor_set(codes_, codes.data(), 0, codes.size() * sizeof(int32_t));
        debug::timing_log_scalar(
            "index_tts2_5.semantic_codec.codes.input_upload_ms",
            engine::debug::elapsed_ms(timing_start, Clock::now()));
        ...
        ggml_backend_synchronize(execution_.backend());
        debug::timing_log_scalar(
            "index_tts2_5.semantic_codec.codes.graph.compute_ms",
            engine::debug::elapsed_ms(timing_start, Clock::now()));
        if (status != GGML_STATUS_SUCCESS) {
            throw std::runtime_error("IndexTTS2.5 semantic codec code graph compute failed");
        }

        IndexTTS25SemanticCodecOutput output;
        output.frames = 2 * frames_;
        output.dims = kHidden;
        output.codes = codes;
        ...
        ggml_backend_tensor_get(
            ...
            output.embedding_channel_first.size() * sizeof(float));
        debug::timing_log_scalar(
            "index_tts2_5.semantic_codec.codes.output_read_ms",
            engine::debug::elapsed_ms(timing_start, Clock::now()));
        return output;
    }
...
std::shared_ptr<const IndexTTS25SemanticCodecWeights> load_index_tts2_5_semantic_codec_weights(
    ...
    if (assets.semantic_codec_weights == nullptr) {
        throw std::runtime_error("IndexTTS2.5 semantic codec requires tensor source");
    }
    auto weights = std::make_shared<IndexTTS25SemanticCodecWeights>();
    weights->store = std::make_shared<engine::core::BackendWeightStore>(
        backend,
        backend_type,
        "index_tts2_5.semantic_codec.weights",
...
IndexTTS25SemanticCodecRuntime::IndexTTS25SemanticCodecRuntime(
    ...
    if (assets_ == nullptr) {
        throw std::runtime_error("IndexTTS2.5 semantic codec runtime requires assets");
    }
    if (graph_arena_bytes_ == 0) {
        throw std::runtime_error("IndexTTS2.5 semantic codec graph arena must be non-zero");
    }
    weights_ = load_index_tts2_5_semantic_codec_weights(
        *assets_,
        execution.backend(),
...
IndexTTS25SemanticCodecRuntime::~IndexTTS25SemanticCodecRuntime() = default;

void IndexTTS25SemanticCodecRuntime::prepare_quantize(int64_t frames) {
    if (execution_ == nullptr) {
        throw std::runtime_error("IndexTTS2.5 semantic codec runtime execution context is missing");
    }
    if (frames <= 0) {
        throw std::runtime_error("IndexTTS2.5 semantic codec quantize prepare requires positive frames");
    }
...
void IndexTTS25SemanticCodecRuntime::prepare_codes(int64_t frames) {
    if (execution_ == nullptr) {
        throw std::runtime_error("IndexTTS2.5 semantic codec runtime execution context is missing");
    }
    if (frames <= 0) {
        throw std::runtime_error("IndexTTS2.5 semantic codec code prepare requires positive frames");
    }
...
IndexTTS25SemanticCodecOutput IndexTTS25SemanticCodecRuntime::quantize(const IndexTTS25SemanticEmbedding & semantic) {
    if (quantize_graph_ == nullptr || quantize_graph_->frames() != semantic.frames) {
        throw std::runtime_error("IndexTTS2.5 semantic codec quantize graph was not prepared for this reference length");
    }
    return quantize_graph_->run(semantic);
}

IndexTTS25SemanticCodecOutput IndexTTS25SemanticCodecRuntime::codes_to_embedding(
    const std::vector<int32_t> & codes,
    int64_t frames) {
    if (codes_graph_ == nullptr || codes_graph_->frames() != frames) {
        throw std::runtime_error("IndexTTS2.5 semantic codec code graph was not prepared for this generation length");
    }
    return codes_graph_->run(codes);
}

void IndexTTS25SemanticCodecRuntime::release_graphs() {
    quantize_graph_.reset();
    codes_graph_.reset();
}
