...
IndexTTS25EmotionVector convert_emotion_json(const std::string & content, const std::string & source_text) {
    IndexTTS25EmotionVector out;
    out.values = {
...
std::shared_ptr<const IndexTTS25QwenEmotionWeights> load_index_tts2_5_qwen_emotion_weights(
    ...
    if (assets.qwen_emotion_weights == nullptr) {
        throw std::runtime_error("IndexTTS2.5 Qwen emotion requires tensor source");
    }
    auto weights = std::make_shared<IndexTTS25QwenEmotionWeights>();
    weights->store = std::make_shared<engine::core::BackendWeightStore>(
        backend,
        backend_type,
        "index_tts2_5.qwen_emotion.weights",
...
IndexTTS25QwenEmotionTokenizer::IndexTTS25QwenEmotionTokenizer(std::shared_ptr<const IndexTTS25Assets> assets) {
    if (assets == nullptr) {
        throw std::runtime_error("IndexTTS2.5 Qwen emotion tokenizer requires assets");
    }
...
std::vector<int32_t> IndexTTS25QwenEmotionTokenizer::encode_chat_prompt(const std::string & text) const {
    ...
    if (trailing_spaces > 0) {
        ...
        if (!space_token.has_value()) {
            throw std::runtime_error("IndexTTS2.5 Qwen emotion tokenizer missing space token");
        }
...
std::string IndexTTS25QwenEmotionTokenizer::decode(const std::vector<int32_t> & token_ids, bool skip_special_tokens) const {
    return tokenizer_->decode(token_ids, skip_special_tokens);
}

int32_t IndexTTS25QwenEmotionTokenizer::eos_token_id() const noexcept {
    return eos_token_id_;
}

int32_t IndexTTS25QwenEmotionTokenizer::think_end_token_id() const noexcept {
    return think_end_token_id_;
}

class IndexTTS25QwenEmotionRuntime::PrefillGraph {
public:
    PrefillGraph(
        core::ExecutionContext & execution,
        std::shared_ptr<const IndexTTS25QwenEmotionWeights> weights,
        ...
        if (prompt_steps_ <= 0) {
            throw std::runtime_error("IndexTTS2.5 Qwen emotion prefill graph requires prompt tokens");
        }
        const auto build_start = Clock::now();
        ggml_init_params params{graph_arena_bytes, nullptr, true};
        ctx_.reset(ggml_init(params));
        if (ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 Qwen emotion prefill graph context");
        }
        ...
        if (input_ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 Qwen emotion prefill input context");
        }
        ...
        if (output_ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 Qwen emotion prefill output context");
        }
        core::ModuleBuildContext ctx{ctx_.get(), "index_tts2_5.qwen_emotion.prefill", execution_.backend_type()};
        core::ModuleBuildContext input_ctx{
            input_ctx_.get(),
            "index_tts2_5.qwen_emotion.prefill.inputs",
            execution_.backend_type()};
        core::ModuleBuildContext output_ctx{
            output_ctx_.get(),
            "index_tts2_5.qwen_emotion.prefill.outputs",
            execution_.backend_type()};
        ...
        if (input_buffer_ == nullptr) {
            throw std::runtime_error("failed to allocate IndexTTS2.5 Qwen emotion prefill input buffer");
        }
        output_buffer_ = ggml_backend_alloc_ctx_tensors(output_ctx_.get(), execution_.backend());
        if (output_buffer_ == nullptr) {
            clear_graph();
            throw std::runtime_error("failed to allocate IndexTTS2.5 Qwen emotion prefill output buffer");
        }
        ...
        if (gallocr_ == nullptr ||
            ...
            clear_graph();
            throw std::runtime_error("failed to allocate IndexTTS2.5 Qwen emotion prefill graph");
        }
        ...
        debug::timing_log_scalar("index_tts2_5.qwen_emotion.prefill.graph.build_ms", engine::debug::elapsed_ms(build_start, Clock::now()));
        debug::trace_log_scalar("index_tts2_5.qwen_emotion.prefill.prompt_tokens", prompt_steps_);
    }
    ...
    bool matches(const IndexTTS25QwenEmotionWeights & weights, ggml_backend_t backend, int64_t prompt_steps) const noexcept {
    ...
    int32_t run(const std::vector<int32_t> & prompt_ids) {
        if (static_cast<int64_t>(prompt_ids.size()) != prompt_steps_) {
            throw std::runtime_error("IndexTTS2.5 Qwen emotion prompt length mismatch");
        }
        auto timing_start = Clock::now();
        ggml_backend_tensor_set(token_ids_, prompt_ids.data(), 0, prompt_ids.size() * sizeof(int32_t));
        debug::timing_log_scalar("index_tts2_5.qwen_emotion.prefill.input_upload_ms", engine::debug::elapsed_ms(timing_start, Clock::now()));
        core::set_backend_threads(execution_.backend(), execution_.config().threads);
        timing_start = Clock::now();
        const ggml_status status = core::compute_backend_graph(execution_.backend(), graph_, nullptr, "IndexTTS2.5 Qwen emotion prefill");
        ggml_backend_synchronize(execution_.backend());
        debug::timing_log_scalar("index_tts2_5.qwen_emotion.prefill.graph.compute_ms", engine::debug::elapsed_ms(timing_start, Clock::now()));
        if (status != GGML_STATUS_SUCCESS) {
            throw std::runtime_error("IndexTTS2.5 Qwen emotion prefill graph compute failed");
        }
        timing_start = Clock::now();
        const auto next_token = core::read_tensor_i32(next_token_);
        debug::timing_log_scalar("index_tts2_5.qwen_emotion.prefill.output_read_ms", engine::debug::elapsed_ms(timing_start, Clock::now()));
        if (next_token.size() != 1) {
            throw std::runtime_error("IndexTTS2.5 Qwen emotion prefill argmax output shape mismatch");
        }
...
class IndexTTS25QwenEmotionRuntime::DecodeGraph {
public:
    DecodeGraph(
        core::ExecutionContext & execution,
        std::shared_ptr<const IndexTTS25QwenEmotionWeights> weights,
        ...
        if (cache_steps_ <= 0) {
            throw std::runtime_error("IndexTTS2.5 Qwen emotion decode graph requires cache capacity");
        }
        const auto build_start = Clock::now();
        ggml_init_params params{graph_arena_bytes, nullptr, true};
        ctx_.reset(ggml_init(params));
        if (ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 Qwen emotion decode graph context");
        }
        ggml_init_params input_params{16ull * 1024ull * 1024ull, nullptr, true};
        input_ctx_.reset(ggml_init(input_params));
        if (input_ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 Qwen emotion decode input context");
        }
        ...
        if (state_ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 Qwen emotion decode state context");
        }
        core::ModuleBuildContext ctx{ctx_.get(), "index_tts2_5.qwen_emotion.decode", execution_.backend_type()};
        core::ModuleBuildContext input_ctx{
            input_ctx_.get(),
            "index_tts2_5.qwen_emotion.decode.inputs",
            execution_.backend_type()};
        core::ModuleBuildContext state_ctx{
            state_ctx_.get(),
            "index_tts2_5.qwen_emotion.decode.state",
            execution_.backend_type()};
        ...
        input_buffer_ = ggml_backend_alloc_ctx_tensors(input_ctx_.get(), execution_.backend());
        if (input_buffer_ == nullptr) {
            throw std::runtime_error("failed to allocate IndexTTS2.5 Qwen emotion decode input buffer");
        }
        state_buffer_ = ggml_backend_alloc_ctx_tensors(state_ctx_.get(), execution_.backend());
        if (state_buffer_ == nullptr) {
            throw std::runtime_error("failed to allocate IndexTTS2.5 Qwen emotion decode state buffer");
        }
        gallocr_ = ggml_gallocr_new(ggml_backend_get_default_buffer_type(execution_.backend()));
        if (gallocr_ == nullptr ||
            !ggml_gallocr_reserve(gallocr_, graph_) ||
            !ggml_gallocr_alloc_graph(gallocr_, graph_)) {
            clear_graph();
            throw std::runtime_error("failed to allocate IndexTTS2.5 Qwen emotion decode graph");
        }
        mask_values_.assign(static_cast<size_t>(cache_steps_ + 1), ggml_fp32_to_fp16(-std::numeric_limits<float>::infinity()));
        debug::timing_log_scalar("index_tts2_5.qwen_emotion.decode.graph.build_ms", engine::debug::elapsed_ms(build_start, Clock::now()));
        debug::trace_log_scalar("index_tts2_5.qwen_emotion.decode.cache_steps", cache_steps_);
    }
    ...
    bool can_run(const IndexTTS25QwenEmotionWeights & weights, ggml_backend_t backend, int64_t required_steps) const noexcept {
    ...
    void import_state(const PrefillGraph & prefill) {
        if (prefill.layer_count() != key_sources_.size() || prefill.prompt_steps() > cache_steps_) {
            throw std::runtime_error("IndexTTS2.5 Qwen emotion decode prefill state shape mismatch");
        }
    ...
    int32_t run_step(int32_t token) {
        if (kv_cache_.valid_steps() >= cache_steps_) {
            throw std::runtime_error("IndexTTS2.5 Qwen emotion decode cache exhausted");
        }
        ...
        core::set_backend_threads(execution_.backend(), execution_.config().threads);
        const ggml_status status = core::compute_backend_graph(execution_.backend(), graph_, nullptr, "IndexTTS2.5 Qwen emotion decode");
        ggml_backend_synchronize(execution_.backend());
        if (status != GGML_STATUS_SUCCESS) {
            throw std::runtime_error("IndexTTS2.5 Qwen emotion decode graph compute failed");
        }
        const auto next_token = core::read_tensor_i32(next_token_);
        if (next_token.size() != 1) {
            throw std::runtime_error("IndexTTS2.5 Qwen emotion decode argmax output shape mismatch");
        }
...
IndexTTS25QwenEmotionRuntime::IndexTTS25QwenEmotionRuntime(
    ...
    : assets_(std::move(assets)),
      ...
      tokenizer_(assets_) {
    if (assets_ == nullptr) {
        throw std::runtime_error("IndexTTS2.5 Qwen emotion runtime requires assets");
    }
    if (prefill_graph_arena_bytes_ == 0 || decode_graph_arena_bytes_ == 0) {
        throw std::runtime_error("IndexTTS2.5 Qwen emotion graph arenas must be non-zero");
    }
    weights_ = load_index_tts2_5_qwen_emotion_weights(
        *assets_,
        execution.backend(),
...
IndexTTS25QwenEmotionRuntime::~IndexTTS25QwenEmotionRuntime() = default;

IndexTTS25EmotionVector IndexTTS25QwenEmotionRuntime::infer(const std::string & text, int64_t max_new_tokens) {
    if (execution_ == nullptr) {
        throw std::runtime_error("IndexTTS2.5 Qwen emotion runtime execution context is missing");
    }
    if (max_new_tokens <= 0) {
        throw std::runtime_error("IndexTTS2.5 Qwen emotion max_new_tokens must be positive");
    }
    ...
    if (!saw_eos && static_cast<int64_t>(generated.size()) >= max_new_tokens) {
        throw std::runtime_error("IndexTTS2.5 Qwen emotion decode reached max_new_tokens before EOS");
    }
    ...
    debug::timing_log_scalar("index_tts2_5.qwen_emotion.decode.run_ms", decode_run_ms);
    debug::trace_log_scalar("index_tts2_5.qwen_emotion.generated_tokens", generated.size());
    return convert_emotion_json(content, text);
}

void IndexTTS25QwenEmotionRuntime::release_graphs() {
    prefill_graph_.reset();
    decode_graph_.reset();
}
