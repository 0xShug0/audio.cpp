...
IndexTTS25Config parse_config(const assets::ResourceBundle & resources) {
    ...
    // The official IndexTTS-2.5 config_v2_5.yaml has no dataset section; these values
    // are parsed for compatibility but not used at inference time.
...
void validate_qwen_emotion_config(const assets::ResourceBundle & resources) {
    ...
    if (json::optional_string(root, "model_type", "") != "qwen3") {
        throw std::runtime_error("IndexTTS2.5 Qwen emotion model must have model_type=qwen3");
    }
    if (json::optional_i64(root, "hidden_size", 0) != 1024 ||
        ...
        throw std::runtime_error("IndexTTS2.5 Qwen emotion model config does not match expected 0.6B architecture");
    }
}

void validate_config(const IndexTTS25Config & config, const assets::ResourceBundle & resources) {
    ...
    if (config.emo_num.empty()) {
        throw std::runtime_error("IndexTTS2.5 config emo_num must not be empty");
    }
    validate_qwen_emotion_config(resources);
}

void validate_gpt_weights(const IndexTTS25Config & config, const assets::TensorSource & source) {
    ...
    assets::require_tensor_shape(source, "lang_embedding.weight", {kIndexTTS25LangEmbeddingRows, config.gpt.model_dim});
    assets::require_tensor_shape(source, "emo_conditioning_encoder.after_norm.weight", {config.gpt.emo_condition_output_size});
}

void validate_s2mel_weights(const IndexTTS25Config & config, const assets::TensorSource & source) {
...
void validate_w2v_stats(const IndexTTS25Config & config, const assets::TensorSource & source) {
...
void validate_semantic_codec_weights(const IndexTTS25Config & config, const assets::TensorSource & source) {
...
void validate_weight_anchors(const IndexTTS25Assets & assets) {
...
std::shared_ptr<const IndexTTS25Assets> load_index_tts2_5_assets(const std::filesystem::path & model_path) {
    auto assets = std::make_shared<IndexTTS25Assets>();
    assets->resources = engine::model_spec::load_resource_bundle(
        model_path,
        engine::model_spec::default_spec_path("index_tts2_5"));
