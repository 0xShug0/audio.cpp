...
core::TensorValue group_norm_1_group(
    ...
    if (!weights.weight.has_value() || !weights.bias.has_value()) {
        throw std::runtime_error("IndexTTS2.5 S2Mel length regulator group norm requires affine weights");
    }
...
core::TensorValue adaptive_rms_norm(
    ...
    const IndexTTS25AdaLayerNormWeights & weights) {
...
core::TensorValue cfm_attention(
    ...
    const IndexTTS25DitLayerWeights & weights) {
...
IndexTTS25AdaLayerNormWeights load_ada_norm(
    engine::core::BackendWeightStore & store,
    const engine::assets::TensorSource & source,
    const std::string & prefix,
    engine::assets::TensorStorageType storage_type) {
...
IndexTTS25LengthRegulatorWeights load_length_regulator(
    engine::core::BackendWeightStore & store,
    const engine::assets::TensorSource & source,
    engine::assets::TensorStorageType matmul_storage_type,
    engine::assets::TensorStorageType conv_storage_type) {
    IndexTTS25LengthRegulatorWeights weights;
...
IndexTTS25S2MelCfmWeights load_cfm(
    engine::core::BackendWeightStore & store,
    const engine::assets::TensorSource & source,
    engine::assets::TensorStorageType matmul_storage_type,
    engine::assets::TensorStorageType conv_storage_type) {
    IndexTTS25S2MelCfmWeights weights;
...
std::shared_ptr<const IndexTTS25S2MelWeights> load_index_tts2_5_s2mel_weights(
    ...
    if (assets.s2mel_weights == nullptr) {
        throw std::runtime_error("IndexTTS2.5 S2Mel requires tensor source");
    }
    auto weights = std::make_shared<IndexTTS25S2MelWeights>();
    weights->store = std::make_shared<engine::core::BackendWeightStore>(
        backend,
        backend_type,
        "index_tts2_5.s2mel.weights",
...
class IndexTTS25S2MelRuntime::GptLayerGraph {
public:
    GptLayerGraph(
        core::ExecutionContext & execution,
        std::shared_ptr<const IndexTTS25S2MelWeights> weights,
        int64_t frames,
        ...
        : execution_(execution),
          ...
          frames_(frames) {
        if (frames_ <= 0) {
            throw std::runtime_error("IndexTTS2.5 S2Mel GPT layer graph requires positive frame count");
        }
        if (weights_ == nullptr) {
            throw std::runtime_error("IndexTTS2.5 S2Mel GPT layer graph requires weights");
        }
        const auto build_start = Clock::now();
        ggml_init_params params{graph_arena_bytes, nullptr, true};
        ctx_.reset(ggml_init(params));
        if (ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 S2Mel GPT layer graph context");
        }
        ...
        if (input_ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 S2Mel GPT layer input context");
        }
        core::ModuleBuildContext ctx{ctx_.get(), "index_tts2_5.s2mel.gpt_layer", execution_.backend_type()};
        core::ModuleBuildContext input_ctx{
            input_ctx_.get(),
            "index_tts2_5.s2mel.gpt_layer.inputs",
        ...
        if (input_buffer_ == nullptr) {
            throw std::runtime_error("failed to allocate IndexTTS2.5 S2Mel GPT layer input buffer");
        }
        ...
        if (gallocr_ == nullptr ||
            ...
            clear_graph();
            throw std::runtime_error("failed to allocate IndexTTS2.5 S2Mel GPT layer graph");
        }
        debug::timing_log_scalar("index_tts2_5.s2mel.gpt_layer.graph.build_ms", engine::debug::elapsed_ms(build_start, Clock::now()));
        debug::trace_log_scalar("index_tts2_5.s2mel.gpt_layer.frames", frames_);
    }
    ...
    IndexTTS25S2MelSequence run(const std::vector<float> & latent) {
        if (static_cast<int64_t>(latent.size()) != frames_ * kGptDim) {
            throw std::runtime_error("IndexTTS2.5 S2Mel GPT layer latent size mismatch");
        }
        auto timing_start = Clock::now();
        ggml_backend_tensor_set(input_, latent.data(), 0, latent.size() * sizeof(float));
        debug::timing_log_scalar("index_tts2_5.s2mel.gpt_layer.input_upload_ms", engine::debug::elapsed_ms(timing_start, Clock::now()));
        ...
        debug::timing_log_scalar("index_tts2_5.s2mel.gpt_layer.graph.compute_ms", engine::debug::elapsed_ms(timing_start, Clock::now()));
        if (status != GGML_STATUS_SUCCESS) {
            throw std::runtime_error("IndexTTS2.5 S2Mel GPT layer graph compute failed");
        }
        IndexTTS25S2MelSequence output;
        output.frames = frames_;
        output.dims = kContentDim;
        timing_start = Clock::now();
        output.values = core::read_tensor_f32(output_);
        debug::timing_log_scalar("index_tts2_5.s2mel.gpt_layer.output_read_ms", engine::debug::elapsed_ms(timing_start, Clock::now()));
        return output;
    }
...
class IndexTTS25S2MelRuntime::LengthRegulatorGraph {
public:
    LengthRegulatorGraph(
        ...
        if (input_frames_ <= 0 || output_frames_ <= 0) {
            throw std::runtime_error("IndexTTS2.5 S2Mel length regulator graph requires positive frame counts");
        }
        if (weights_ == nullptr) {
            throw std::runtime_error("IndexTTS2.5 S2Mel length regulator graph requires weights");
        }
        const auto build_start = Clock::now();
        ggml_init_params params{graph_arena_bytes, nullptr, true};
        ctx_.reset(ggml_init(params));
        if (ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 S2Mel length regulator graph context");
        }
        ggml_init_params input_params{16ull * 1024ull * 1024ull, nullptr, true};
        input_ctx_.reset(ggml_init(input_params));
        if (input_ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 S2Mel length regulator input context");
        }
        core::ModuleBuildContext ctx{ctx_.get(), "index_tts2_5.s2mel.length_regulator", execution_.backend_type()};
        core::ModuleBuildContext input_ctx{
            input_ctx_.get(),
            "index_tts2_5.s2mel.length_regulator.inputs",
            execution_.backend_type()};
        ...
        input_buffer_ = ggml_backend_alloc_ctx_tensors(input_ctx_.get(), execution_.backend());
        if (input_buffer_ == nullptr) {
            throw std::runtime_error("failed to allocate IndexTTS2.5 S2Mel length regulator input buffer");
        }
        gallocr_ = ggml_gallocr_new(ggml_backend_get_default_buffer_type(execution_.backend()));
        if (gallocr_ == nullptr ||
            !ggml_gallocr_reserve(gallocr_, graph_) ||
            !ggml_gallocr_alloc_graph(gallocr_, graph_)) {
            clear_graph();
            throw std::runtime_error("failed to allocate IndexTTS2.5 S2Mel length regulator graph");
        }
        ...
        core::write_tensor_f32(
            ...
            mask_values_);
        debug::timing_log_scalar("index_tts2_5.s2mel.length_regulator.graph.build_ms", engine::debug::elapsed_ms(build_start, Clock::now()));
        debug::trace_log_scalar("index_tts2_5.s2mel.length_regulator.input_frames", input_frames_);
        debug::trace_log_scalar("index_tts2_5.s2mel.length_regulator.output_frames", output_frames_);
    }
    ...
    IndexTTS25S2MelSequence run(const std::vector<float> & content) {
        if (static_cast<int64_t>(content.size()) != input_frames_ * kContentDim) {
            throw std::runtime_error("IndexTTS2.5 S2Mel length regulator content size mismatch");
        }
        auto timing_start = Clock::now();
        ggml_backend_tensor_set(input_, content.data(), 0, content.size() * sizeof(float));
        debug::timing_log_scalar("index_tts2_5.s2mel.length_regulator.input_upload_ms", engine::debug::elapsed_ms(timing_start, Clock::now()));
        core::set_backend_threads(execution_.backend(), execution_.config().threads);
        timing_start = Clock::now();
        const ggml_status status = core::compute_backend_graph(execution_.backend(), graph_);
        ggml_backend_synchronize(execution_.backend());
        debug::timing_log_scalar("index_tts2_5.s2mel.length_regulator.graph.compute_ms", engine::debug::elapsed_ms(timing_start, Clock::now()));
        if (status != GGML_STATUS_SUCCESS) {
            throw std::runtime_error("IndexTTS2.5 S2Mel length regulator graph compute failed");
        }
        IndexTTS25S2MelSequence output;
        ...
        timing_start = Clock::now();
        output.values = core::read_tensor_f32(output_);
        debug::timing_log_scalar("index_tts2_5.s2mel.length_regulator.output_read_ms", engine::debug::elapsed_ms(timing_start, Clock::now()));
        return output;
    }
...
class IndexTTS25S2MelRuntime::CfmGraph {
public:
    CfmGraph(
        core::ExecutionContext & execution,
        std::shared_ptr<const IndexTTS25S2MelWeights> weights,
        ...
        : execution_(execution),
          ...
          batch_(use_cfg ? 2 : 1) {
        if (frames_ <= 0) {
            throw std::runtime_error("IndexTTS2.5 S2Mel CFM graph requires positive frame count");
        }
        if (weights_ == nullptr) {
            throw std::runtime_error("IndexTTS2.5 S2Mel CFM graph requires weights");
        }
        const auto build_start = Clock::now();
        ggml_init_params params{graph_arena_bytes, nullptr, true};
        ctx_.reset(ggml_init(params));
        if (ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize IndexTTS2.5 S2Mel CFM graph context");
        }
        ggml_init_params input_params{16ull * 1024ull * 1024ull, nullptr, true};
        input_ctx_.reset(ggml_init(input_params));
        if (input_ctx_ == nullptr) {
        ...
        if (gallocr_ == nullptr ||
            ...
            !ggml_gallocr_alloc_graph(gallocr_, graph_)) {
            clear_graph();
            throw std::runtime_error("failed to allocate IndexTTS2.5 S2Mel CFM graph");
        }
        ...
        debug::timing_log_scalar("index_tts2_5.s2mel.cfm.graph.build_ms", engine::debug::elapsed_ms(build_start, Clock::now()));
        debug::trace_log_scalar("index_tts2_5.s2mel.cfm.frames", frames_);
        debug::trace_log_scalar("index_tts2_5.s2mel.cfm.batch", batch_);
    }
    ...
    std::vector<float> run(
        ...
        if (static_cast<int64_t>(x.size()) != mel_values ||
            ...
            throw std::runtime_error("IndexTTS2.5 S2Mel CFM graph input shape mismatch");
        }
        auto timing_start = Clock::now();
        ...
        debug::timing_log_scalar("index_tts2_5.s2mel.cfm.input_upload_ms", engine::debug::elapsed_ms(timing_start, Clock::now()));
        core::set_backend_threads(execution_.backend(), execution_.config().threads);
        timing_start = Clock::now();
        const ggml_status status = core::compute_backend_graph(execution_.backend(), graph_);
        ggml_backend_synchronize(execution_.backend());
        debug::timing_log_scalar("index_tts2_5.s2mel.cfm.graph.compute_ms", engine::debug::elapsed_ms(timing_start, Clock::now()));
        if (status != GGML_STATUS_SUCCESS) {
            throw std::runtime_error("IndexTTS2.5 S2Mel CFM graph compute failed");
        }
...
void zero_prompt_region(std::vector<float> & values, int64_t channels, int64_t frames, int64_t prompt_frames) {
    if (prompt_frames < 0 || prompt_frames > frames) {
        throw std::runtime_error("IndexTTS2.5 S2Mel CFM prompt frame count is out of range");
    }
...
std::vector<float> make_prompt_x(
    ...
    if (static_cast<int64_t>(prompt.size()) != channels * prompt_frames) {
        throw std::runtime_error("IndexTTS2.5 S2Mel CFM reference mel shape mismatch");
    }
    for (int64_t c = 0; c < channels; ++c) {
        std::copy_n(
...
std::vector<float> make_condition_with_prompt(
    ...
    if (static_cast<int64_t>(condition.size()) != frames * kHidden) {
        throw std::runtime_error("IndexTTS2.5 S2Mel CFM condition shape mismatch");
    }
    return condition;
}

IndexTTS25S2MelRuntime::IndexTTS25S2MelRuntime(
    ...
    if (assets_ == nullptr) {
        throw std::runtime_error("IndexTTS2.5 S2Mel runtime requires assets");
    }
    if (graph_arena_bytes_ == 0) {
        throw std::runtime_error("IndexTTS2.5 S2Mel graph arena must be non-zero");
    }
    weights_ = load_index_tts2_5_s2mel_weights(
        *assets_,
        execution.backend(),
...
IndexTTS25S2MelRuntime::~IndexTTS25S2MelRuntime() = default;

void IndexTTS25S2MelRuntime::prepare_gpt_layer(int64_t frames) {
    if (execution_ == nullptr) {
        throw std::runtime_error("IndexTTS2.5 S2Mel runtime execution context is missing");
    }
...
void IndexTTS25S2MelRuntime::prepare_length_regulator(int64_t input_frames, int64_t output_frames) {
    if (execution_ == nullptr) {
        throw std::runtime_error("IndexTTS2.5 S2Mel runtime execution context is missing");
    }
...
void IndexTTS25S2MelRuntime::prepare_cfm(int64_t total_frames, bool use_cfg) {
    if (execution_ == nullptr) {
        throw std::runtime_error("IndexTTS2.5 S2Mel runtime execution context is missing");
    }
    if (total_frames <= 0) {
        throw std::runtime_error("IndexTTS2.5 S2Mel CFM prepare requires positive frame count");
    }
...
void IndexTTS25S2MelRuntime::release_pre_cfm_graphs() {
    gpt_layer_graph_.reset();
    length_regulator_graph_.reset();
}

void IndexTTS25S2MelRuntime::release_cfm_graph() {
    cfm_graph_.reset();
}

IndexTTS25S2MelSequence IndexTTS25S2MelRuntime::project_gpt_latent(const std::vector<float> & latent, int64_t frames) {
    if (gpt_layer_graph_ == nullptr || gpt_layer_graph_->frames() != frames) {
        throw std::runtime_error("IndexTTS2.5 S2Mel GPT layer graph was not prepared for this latent length");
    }
    return gpt_layer_graph_->run(latent);
}

IndexTTS25S2MelSequence IndexTTS25S2MelRuntime::regulate_length(
    const std::vector<float> & content,
    int64_t input_frames,
    int64_t output_frames) {
    if (length_regulator_graph_ == nullptr || !length_regulator_graph_->matches(input_frames, output_frames)) {
        throw std::runtime_error("IndexTTS2.5 S2Mel length regulator graph was not prepared for this shape");
    }
    return length_regulator_graph_->run(content);
}

IndexTTS25S2MelMel IndexTTS25S2MelRuntime::infer_mel(
    const std::vector<float> & condition,
    ...
    if (condition.empty() || reference_mel.empty() || style.empty()) {
        throw std::runtime_error("IndexTTS2.5 S2Mel CFM requires non-empty condition, reference mel, and style");
    }
    if (total_frames <= 0 || reference_frames <= 0 || reference_frames > total_frames) {
        throw std::runtime_error("IndexTTS2.5 S2Mel CFM frame counts are invalid");
    }
    if (diffusion_steps <= 0) {
        throw std::runtime_error("IndexTTS2.5 S2Mel CFM diffusion steps must be positive");
    }
    if (static_cast<int64_t>(style.size()) != kStyleDim) {
        throw std::runtime_error("IndexTTS2.5 S2Mel CFM style shape mismatch");
    }
    const bool use_cfg = cfg_rate > 0.0F;
    if (cfm_graph_ == nullptr || !cfm_graph_->matches(total_frames, use_cfg)) {
        throw std::runtime_error("IndexTTS2.5 S2Mel CFM graph was not prepared for this shape");
    }
    const auto rng_policy = engine::sampling::resolve_torch_cuda_sampling_policy(
        execution_->backend_type(),
        execution_->config().device,
        "index_tts2_5.s2mel.cuda_sampling_policy",
        "IndexTTS2.5",
    ...
    debug::timing_log_scalar("index_tts2_5.s2mel.cfm.euler_graph_ms", graph_ms);
    debug::timing_log_scalar("index_tts2_5.s2mel.cfm.euler_total_ms", engine::debug::elapsed_ms(cfm_start));
    return out;
}
