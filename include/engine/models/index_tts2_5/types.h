...
// Rows in the GPT lang_embedding table. indextts/utils/tokenizer.py defines
// 106 language codes (including "common"); the checkpoint table has one extra
// unused row.
constexpr int64_t kIndexTTS25LangEmbeddingRows = 107;

struct IndexTTS25GptConfig {
...
struct IndexTTS25SemanticCodecConfig {
...
struct IndexTTS25Config {
    ...
    IndexTTS25GptConfig gpt;
    IndexTTS25SemanticCodecConfig semantic_codec;
    IndexTTS25S2MelConfig s2mel;
    std::vector<int64_t> emo_num;
};

struct IndexTTS25GenerationOptions {
    bool do_sample = true;
    float top_p = 0.8F;
    int top_k = 30;
