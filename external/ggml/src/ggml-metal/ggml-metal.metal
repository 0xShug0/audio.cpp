...
kernel void kernel_cpy_t_t(
    ...
    // The last threadgroup can contain padded rows (including ne01 == 1
    // after a permutation). Those threads must not read or overwrite the
    // next channel/batch through its different source strides.
    if (i01 >= args.ne01) {
        return;
    }
