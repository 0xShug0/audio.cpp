...
std::string joinVoicePaths(NSArray<NSURL *> *voiceURLs) {
    std::ostringstream joined;
    for (NSUInteger index = 0; index < voiceURLs.count; ++index) {
        if (index > 0) {
            joined << ",";
        }
        joined << pathString(voiceURLs[index]);
    }
    return joined.str();
}
...
- (void)generateWithText:(NSString *)text
    ...
    NSArray<NSURL *> *voicesCopy = [voiceURLs copy];
...
- (VibeVoiceGenerationResult *)generateSynchronouslyWithText:(NSString *)text
    ...
    if (voiceURLs.count == 0) {
        throw std::runtime_error("at least one voice sample is required");
    }
    if (inferenceSteps <= 0) {
    ...
    request.options["voice_samples"] = joinVoicePaths(voiceURLs);
    ...
    request.options["voice_prompt_max_seconds"] = "5";
