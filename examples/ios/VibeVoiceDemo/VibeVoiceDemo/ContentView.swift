...
struct ContentView: View {
    ...
    private let terminalBackground = Color(red: 0.015, green: 0.02, blue: 0.015)
    private let terminalPanel = Color(red: 0.035, green: 0.055, blue: 0.04)
    private let terminalGreen = Color(red: 0.34, green: 1.0, blue: 0.48)
    private let terminalAccent = Color(red: 0.10, green: 0.46, blue: 0.20)
    private let terminalDim = Color(red: 0.50, green: 0.72, blue: 0.54)
    private let terminalBorder = Color(red: 0.12, green: 0.42, blue: 0.20)
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                VStack(spacing: 14) {
                    Text("> VibeVoice audio.cpp IOS demo")
                    ...
                    TextEditor(text: $text)
                        ...
                        .padding(10)
                        .frame(minHeight: 220)
                        .background(terminalPanel, in: RoundedRectangle(cornerRadius: 8))
                        .overlay(
                    ...
                    .padding(10)
                    .background(terminalPanel.opacity(0.72), in: RoundedRectangle(cornerRadius: 8))
                    .overlay(
                        RoundedRectangle(cornerRadius: 8)
                            .stroke(terminalBorder.opacity(0.8), lineWidth: 1)
                    )
                    .padding(.top, 4)
            ...
            .navigationTitle("VibeVoice audio.cpp IOS demo")
    ...
    private var playbackProgress: Double {
        ...
        return min(max(playbackSeconds / playbackDuration, 0), 1)
    }
    ...
    private func generate() {
        ...
        VibeVoiceEngine.shared().generate(
            ...
            voiceStateURLs: resources.voiceStateURLs,
    ...
    private func preloadVoiceStates() {
        ...
        status = "Loading voice states"
        VibeVoiceEngine.shared().preload(
            withModelURL: resources.modelURL,
            voiceStateURLs: resources.voiceStateURLs
        ) { error in
            if let error {
                status = error.localizedDescription
    ...
    private func bundledResources() -> (modelURL: URL, voiceStateURLs: [URL])? {
        guard let modelURL = Bundle.main.url(
            forResource: "vibevoice-1.5b-q4-ios",
            withExtension: "gguf"
        ) else {
        ...
        let voiceStateURLs = voiceStateNames.compactMap {
            Bundle.main.url(forResource: $0, withExtension: "vvstate")
        }
        guard voiceStateURLs.count == voiceStateNames.count else {
            status = "Voice state resources missing"
            return nil
        }
        return (modelURL, voiceStateURLs)
    }

    private func generateRandomText() {
        let sentenceCount = Int.random(in: 4...10)
    ...
    private func play() {
        ...
        do {
            ...
            nextPlayer.play()
            player = nextPlayer
            playbackDuration = nextPlayer.duration
            playbackSeconds = nextPlayer.currentTime
            isPlaying = true
        } catch {
            status = error.localizedDescription
        }
    ...
    private func updatePlaybackProgress() {
        ...
        if !player.isPlaying && isPlaying {
            isPlaying = false
            playbackSeconds = player.duration
        }
    }
